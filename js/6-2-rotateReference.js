...
function create(){
	...
	game.time.events.loop(Phaser.Timer.SECOND*4, patrol_entity_e_2_XX__XX__XX__patrol_entity_e_2_XX__XX__XX_Listener, this);

	addedEntities['e_1_XX_'].forEach(function(item){item.body.immovable=true;}, this);
	addedEntities['e_3_XX_'].forEach(function(item){item.body.immovable=true;}, this);
...
function update(){
	...
	if (!game.physics.arcade.overlap(addedEntities['e_2_XX_'],addedEntities['e_2_XX_'],null,null, this)){o_2_XX_NotOverlapHandler();}
	addedEntities['e_2_XX_'].forEach(function(item){
		item.inputEnabled=true;
		item.events.onInputDown.add(o_3_XX__e_2_XX_ClickListener,this);
	}, this);

	addedEntities['e_3_XX_'].forEach(function(item){
		item.inputEnabled=true;
		item.events.onInputDown.add(o_4_XX__e_3_XX_ClickListener,this);
	}, this);

	game.physics.arcade.overlap(addedEntities['e_2_XX_'],addedEntities['e_3_XX_'],o_6_XX_OverlapHandler,null, this);
	if(r_2_XX_>=7){
		addedEntities['e_2_XX_'].forEach(function(item) {
		addedEntities['e_2_XX_'].forEach(function(item2) {
	...
	addedEntities['e_2_XX_'].forEach(function(item) {
		addedEntities['e_3_XX_'].forEach(function(item2) {
		var tempPoint = new Phaser.Point(item2.x-item.x,item2.y-item.y);
		tempPoint.normalize();
		tempPoint.x *= 10;
		tempPoint.y *= 10;
	...
	game.physics.arcade.collide(e_2_XX_,e_3_XX_,null,null,this);
	for(var k in addedEntities) {if (addedEntities.hasOwnProperty(k)) {
		var entity = addedEntities[k];
		entity.forEach(function(item) {
...
function o_2_XX_NotOverlapHandler(){
	r_2_XX_=r_2_XX_+5*this.game.time.elapsed/3840.0;

		
};

function o_3_XX__e_2_XX_ClickListener(){
	var x=50+ xOffset;var y=50+ yOffset;for (var ii = 0; ii < 1; ii++){
		x+=(Math.random() * 30) - 15;
		y+=(Math.random() * 30) - 15;
		initEntity(addedEntities['e_1_XX_'].create(x,y,'e_1_XX_'));
...
function o_4_XX__e_3_XX_ClickListener(){
	r_2_XX_=r_2_XX_+1/2;

		
};

function o_6_XX_OverlapHandler(e1,e2){
...
function patrol_entity_e_2_XX__XX__XX__patrol_entity_e_2_XX__XX__XX_Listener(){
	addedEntities['e_2_XX_'].forEach(function(item){item.angle += 20;}, this);
	r_1_XX_=r_1_XX_-1/2;
...
function o_1_XX__e_1_XX_ClickListener(entity){
	if(!game.physics.arcade.overlap(addedEntities['e_2_XX_'], addedEntities['e_2_XX_'], null, null, this)){
	o_1_XX_NotOverlapHandler();
	}
};
