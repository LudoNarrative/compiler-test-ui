...
function create(){
	...
	game.time.events.loop(Phaser.Timer.SECOND*4, o_4_XX__t_1_XX_Listener, this);
...
function update(){
	...
	//updating program values!
	if(State.get('debugMode') === true){
		console.log("here? baby")
	for (var key in labels) {
	  if (labels.hasOwnProperty(key)) {
	    console.log(key + ' -> ' + labels[key]);
	    if(labels[key] === 'satiation'){
	      setVariable('satiation', labels[key]);
	    }
	  }
	}
}
