...
var e2;
function preload(){
	game.load.image('e1','assets/sprites/newArt/Triangle/triangleStyled01.png');

	game.load.image('e2','assets/sprites/newArt/Triangle/triangleCircleCut.png');
};
...
function update(){
	...
	addedEntities['e2'].forEach(function(item){item.tint=0xff0000;}, this);
	for(var k in addedEntities) {if (addedEntities.hasOwnProperty(k)) {
		var entity = addedEntities[k];
		entity.forEach(function(item) {
...
function o1OverlapHandler(e1,e2){
	...
	if (typeof e1 !== 'undefined' && e1.key === 'e1'){
		e1.health = e1.health-10/2;
	}
	if (typeof e2 !== 'undefined' && e2.key === 'e1'){
		e2.health = e2.health-10/2;
	}
