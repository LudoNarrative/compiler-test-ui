...
function create(){
	...
	graphics.drawRoundedRect(0,0, 400, 300, 10);
	graphics.endFill();
	graphics.alpha = 1;
	sprite = game.add.sprite(xOffset +200, yOffset + 150, graphics.generateTexture());	sprite.alpha = 0.5;
	...
	bmd = game.add.bitmapData(400	, 300);
	brush = game.make.sprite(0,0,'e_1_XX_');
	
	bmd_graphic = game.add.graphics( 0,0);
	bmd_graphic.beginFill(0xfdd9a6);
	bmd_graphic.drawRoundedRect(xOffset,yOffset, 400, 300, 10);
	bmd_graphic.endFill();
	bmd_graphic.alpha = 0.5;
	bmd_graphic = game.make.sprite(0,0,bmd_graphic.generateTexture());
	
	//	Black and opaque
	bmd.fill(1,1,1,1);
	bmd.addToWorld(xOffset,yOffset);
	bmd.draw(bmd_graphic, 0 ,0 , null, null,'source-in');

	bmd.draw(brush, 100, 100, null, null, 'destination-out');
...
bad_sound =  game.add.audio('bad_sound');
	...
	graphics = game.add.graphics(155, 1);
graphics.beginFill(0x000000);
graphics.drawRect(0,0, 5, 18);
graphics.alpha = 0.5;
graphics.endFill();
...
function update(){
	...
	addedEntities['e_2_XX_'].forEach(function(item){item.health = item.health+3/160;}, this);
	...
	if(game.input.activePointer.leftButton.isDown){
		addedEntities['e_2_XX_'].forEach(function(item){item.health = item.health-1/160;}, this);
	...
	addedEntities['e_1_XX_'].forEach(function(item) {
		bmd.draw(brush, item.x-xOffset-item.width/2, item.y-yOffset-item.height/2, null, null, 'destination-out');
		move_left(item,7);
}, this);
