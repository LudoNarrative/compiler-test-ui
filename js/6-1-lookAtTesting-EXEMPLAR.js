...
var r_2_XX_;
function preload(){
	//game.load.image('e_1_XX_','assets/sprites/square.png');
	game.load.image('e_1_XX_','assets/sprites/circle.png');

	//game.load.image('e_2_XX_','assets/sprites/triangle.png');
...
function create(){
	...
	game.input.onDown.add(click_to_chase_entity_e_2_XX__XX__XX_PressedHandler, this);
	game.time.events.loop(Phaser.Timer.SECOND*2, o_3_XX__t_1_XX_Listener, this);

	game.time.events.loop(Phaser.Timer.SECOND*2, o_4_XX__t_2_XX_Listener, this);
...
function update(){
	...
	if(game.input.activePointer.leftButton.isDown && r_2_XX_<=0){
		...
		tempPoint.x *= 10;
		tempPoint.y *= 10;
		console.log("Um, I think this is the code that has the thing move towards the other thing...?")
...
function click_to_chase_entity_e_2_XX__XX__XX_PressedHandler(){
	if(r_1_XX_<=4){
		var x=50+ xOffset;var y=50+ yOffset;for (var ii = 0; ii < 1; ii++){
		x+=(Math.random() * 30) - 15;
		y+=(Math.random() * 30) - 15;
		...
		//Let's do it again in the function: //AM I EVEN BEING ADDED TO WHERE I HOPE I AM MAYBE? I want e_2_XX_ to look at e_1_XX_//don't forget this equation: O.angle = Math.atan2(Other.y- E.y, Other.x - E.x);
		//var newAngle = Math.atan2(addedEntities['e_1_XX_'].y - addedEntities['e_2_XX_'].y, addedEntities['e_1_XX_'].x - addedEntities['e_2_XX_'].x);
		


		addedEntities['e_1_XX_'].forEach(function(item) {
				var newAngle = Math.atan2(game.input.mousePointer.y - item.y, game.input.mousePointer.x - item.x);
				console.log("here is my fancy newangle code: " , newAngle);
				//item.angle = newAngle;
/*
				var tempPoint = new Phaser.Point(game.input.mousePointer.x-item.x,game.input.mousePointer.y-item.y);
				tempPoint.normalize();
		...
		//console.log("addedEntities e_1_XX.y: " , addedEntities['e_1_XX_'].y);
		//console.log("here is my newAngle... " , newAngle);
		}
};
