...
function create(){
	...
	game.input.onDown.add(o3PressedHandler, this);
	game.time.events.loop(Phaser.Timer.SECOND*5, t2_t2Listener, this);

	game.input.onDown.add(o1PressedHandler, this);
	game.time.events.loop(Phaser.Timer.SECOND*10, t1_t1Listener, this);

	addedEntities['e1'].forEach(function(item){item.angle = Math.random() * (360-0) + 0;}, this);
	addedEntities['e2'].forEach(function(item){item.angle = Math.random() * (360-0) + 0;}, this);
...
function update(){
	...
	bmd2.ctx.fill();
	resourceOneCurrentBar = game.add.sprite(10, 10, bmd2);
	resourceOneCurrentBar.anchor.setTo(0.5, 0.5);
